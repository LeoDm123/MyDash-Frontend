...
interface ExpensesMiniChartProps {
    dataset: Dataset
    previousDataset?: Dataset | null
    locale?: string // "es-AR"
}

const ExpensesMiniChart: React.FC<ExpensesMiniChartProps> = ({
    dataset,
    previousDataset,
    locale = 'es-AR',
}) => {
    ...
    // Agrupar por día y construir acumulado de egresos
    ...
    const chartData = currentDays.map((d, i) => {
        ...
        return {
            date: d.date,
            current: d.egresos,
            previous: prevPoint ? prevPoint.egresos : null,
        }
    })
    ...
    const currentValue = currentDays.reduce((s, it) => s + it.egresos, 0)
    const previousValue = prevDays
        .slice(0, len)
        .reduce((s, it) => s + it.egresos, 0)
            ...
            <div className="w-4/5 h-24">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart
                        data={chartData}
                        onClick={(s: any) => {
                            const dateYmd = s?.activeLabel as string | undefined
                            if (!dateYmd) return
